...
class Elixir():
   ...
   def find_region_overlap(self): # Find and indicate the overlap between regions of two SEGMENTs
   ...
   def pearson_corr_coef(self): # Indicate pcc of two FUNCTIONs
       if self.path_to_file1[-1] == 'f' and self.path_to_file2[-1] =='f':
           ...
           self.pearson_correlation_coefficient = 'Warning: In order to calculate ' \
            'Pearson Correlation Coefficient, both inputs must be in "FUNCTION" formant'
           
   def seg_func_mean(self): #Calculate and indicate the mean of FUNCTION corresponding to the SEGMENT 
       if self.path_to_file1[-1] == 's' and self.path_to_file2[-1] =='f':
           ...
           self.segment_function_mean = 'Warning: SEGMENT file and FUNCTION file must be entered, respectively'
